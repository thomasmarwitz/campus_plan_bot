...
def evaluate_rag_multi_turn():
    """
    Evaluates the RAG system on multi-turn conversations.
    """
    ...
    results = []
    for interaction in tqdm(evaluation_data, desc="Evaluating RAG on multi-turn conversations"):
        for i, turn in enumerate(interaction["prompts"]):
            prompt = turn["reformulated_prompt"]
            expected_id = str(turn["input_data"])
            
            retrieved_docs = rag.retrieve_context(prompt, limit=5) 
    ...
    output_path = output_dir / "rag_evaluation.csv"
    ...
    overall_success_ratio = successful_retrievals / total_prompts if total_prompts > 0 else 0

    print(f"Overall success ratio: {overall_success_ratio:.2f} ({successful_retrievals}/{total_prompts})")
    ...
    for turn_id in turns:
        ...
        print(f"  Turn {turn_id} success ratio: {turn_success_ratio:.2f} ({turn_successful}/{turn_total})")
